...
export function HomePage({ onOpenProject, onNewProject, workspaceName }) {
 ...
 style={{ background: COLORS.surfaceHover, border: `1px solid ${COLORS.border}`, borderRadius: 16, padding: 18, cursor: 'pointer', transition: 'background 0.15s, border-color 0.15s, box-shadow 0.15s' }}
 className="proj-card">
 ...
 {myTasks.length === 0 ? (
 <div style={{ background: COLORS.surfaceHover, border: `1px solid ${COLORS.borderStrong}`, borderRadius: 16, padding: '28px 20px', textAlign: 'center' }}>
...
export function ProjectView({ project, toast }) {
 ...
 <select value={filterS} onChange={e => setFilterS(e.target.value)} style={{ background: COLORS.surfaceHover, border: `1px solid ${COLORS.borderStrong}`, color: COLORS.textDim, borderRadius: 8, padding: '4px 8px', fontSize: 12, outline: 'none' }}>
...
function PipelineCard({ project: p, converting, onEdit, onConvert, onDelete }) {
 const [confirmDel, setConfirmDel] = useState(false)
 return (
 <div style={{ background: COLORS.surfaceHover, border: `1px solid ${COLORS.border}`, borderRadius: 16, padding: 20, display: 'flex', flexDirection: 'column' }}>
