...
export default function TaskModal({ task, projectId, isAdmin, onClose, toast }) {
 ...
 return (
 <button key={k} onClick={() => setPriority(k)} style={{ flex: 1, padding: '10px 8px', borderRadius: 10, border: `2px solid ${active ? v.color : COLORS.border}`, background: active ? v.color + '25' : COLORS.surface, color: active ? v.color : COLORS.textMuted, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 700, fontSize: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, transition: 'all 0.15s' }}>
 ...
 return (
 <button key={m.user_id} onClick={() => setAssigneeId(m.user_id)} style={{ padding: '6px 14px', borderRadius: 20, border: `2px solid ${active ? COLORS.accent : COLORS.border}`, background: active ? COLORS.accent + '25' : COLORS.surface, color: active ? COLORS.accent : COLORS.textDim, cursor: 'pointer', fontFamily: 'inherit', fontWeight: active ? 700 : 400, fontSize: 12, transition: 'all 0.15s', display: 'flex', alignItems: 'center', gap: 6 }}>
 ...
 <div style={{ flex: 1, background: COLORS.surface, border: `1px solid ${COLORS.borderStrong}`, borderRadius: 10, padding: '9px 12px' }}>
