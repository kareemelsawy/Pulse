...
export function BoardView({ tasks, onTaskClick }) {
 ...
 return (
 <div key={sk} className="card" style={{ flex: '0 0 264px', borderRadius: 12, overflow: 'hidden' }}>
 ...
 style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 10, padding: '12px 14px', cursor: 'pointer', transition: 'background 0.15s, border-color 0.15s, box-shadow 0.15s' }}
 onMouseEnter={e => { e.currentTarget.style.background = COLORS.surfaceHover; e.currentTarget.style.borderColor = 'rgba(255,255,255,0.20)'; e.currentTarget.style.boxShadow = '0 4px 24px rgba(0,100,255,0.12)' }}
 onMouseLeave={e => { e.currentTarget.style.background = COLORS.surface; e.currentTarget.style.borderColor = COLORS.surfaceHover; e.currentTarget.style.boxShadow = '' }}>
...
export function ListView({ tasks, onTaskClick }) {
 ...
 <div style={{ display: 'grid', gridTemplateColumns: '1fr 110px 120px 90px 90px', padding: '9px 15px', borderBottom: `1px solid ${COLORS.border}`, fontSize: 11, fontWeight: 700, color: COLORS.textMuted, letterSpacing: '0.06em', textTransform: 'uppercase', background: COLORS.surfaceHover }}>
