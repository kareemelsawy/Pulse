...
function glassCard() {
  const light = isLight()
  return {
    background:         light ? 'rgba(255,255,255,0.58)' : 'rgba(255,255,255,0.07)',
...
export function ProgressBar({ tasks = [] }) {
  ...
  return (
    <div>
      ...
      <div style={{ height: 4, borderRadius: 99, background: 'rgba(255,255,255,0.10)', overflow:'hidden', position:'relative' }}>
        <div style={{ position:'absolute', inset:0, background: 'rgba(255,255,255,0.05)', backdropFilter:'blur(4px)' }} />
        <div style={{
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        background: isLight() ? 'rgba(255,255,255,0.82)' : 'rgba(18,14,50,0.80)',
        backdropFilter:'blur(40px) saturate(200%)',
        WebkitBackdropFilter:'blur(40px) saturate(200%)',
        border:`1px solid ${isLight() ? 'rgba(255,255,255,0.92)' : 'rgba(255,255,255,0.16)'}`,
        borderRadius: 20,
        padding: 28,
        width, maxWidth:'100%',
    ...
    <div style={{ position:'fixed', bottom:24, right:24, zIndex:300, display:'flex', flexDirection:'column', gap:8 }}>
      {toasts.map(t => (
        <div key={t.id} onClick={() => onRemove(t.id)} style={{
          background: isLight() ? 'rgba(255,255,255,0.78)' : 'rgba(20,16,55,0.82)',
          backdropFilter: 'blur(30px) saturate(200%)',
          WebkitBackdropFilter: 'blur(30px) saturate(200%)',
          border: `1px solid ${t.type==='error' ? COLORS.red+'40' : t.type==='success' ? COLORS.green+'40' : 'rgba(255,255,255,0.16)'}`,
          ...
          <div style={{
            ...
            background: t.type==='error' ? COLORS.red+'25' : t.type==='success' ? COLORS.green+'25' : 'rgba(255,255,255,0.10)',
...
export function Divider({ style: s }) {
  return <div style={{ height:1, background:'rgba(255,255,255,0.09)', ...s }} />
}

// ── Input/Label styles ───────────────────────────────────────────────────────
export const iStyle = {
  get width()      { return '100%' },
  get background() { return isLight() ? 'rgba(255,255,255,0.65)' : 'rgba(255,255,255,0.07)' },
  get border()     { return `1px solid ${isLight() ? 'rgba(120,100,200,0.20)' : 'rgba(255,255,255,0.12)'}` },
  borderRadius: 10,
  padding: '9px 13px',
