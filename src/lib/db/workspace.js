...
// Stores pending invitations in workspace_invitations table.
// When user signs up / signs in, App.jsx checks for a pending invite token.

export async function sendWorkspaceInvite(workspaceId, inviterName, email, role, notifSettings) {
  email = email.toLowerCase().trim()

  // Check if already a member via profiles table
  ...
  if (profile) {
    ...
    const { data: existing } = await supabase
      ...
      .single()
    if (existing) throw new Error('This person is already a workspace member.')
  }
