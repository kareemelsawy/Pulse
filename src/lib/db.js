...
// ─── Guest Invitations ────────────────────────────────────────────────────────
// Tracks @homzmart.com emails that have been assigned tasks but aren't members
...
export async function logGuestInvitation(workspaceId, email, taskTitle, taskId) {
  ...
  const { data: existing } = await supabase
    ...
    .eq('workspace_id', workspaceId)
    .limit(1)
  // We can't filter by email here because workspace_members stores user_id
  // Instead we log to guest_invitations for tracking
  const { error } = await supabase
...
// Check if user is a workspace member (vs guest)
export async function checkIsMember(userId, email) {
  // Check by user_id in workspace_members
...
// Get all meetings accessible to this guest by email
export async function getGuestMeetings(email) {
  const { data, error } = await supabase
    .from('project_meetings')
