...
export function buildWorkspaceInviteEmail({ inviterName, workspaceName, role, inviteUrl, appName = 'Pulse' }) {
  const roleLabel = { owner: 'Owner', pm: 'Project Manager', user: 'Member' }[role] || role
  const html = `<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"></head>
<body style="margin:0;padding:20px;background:#0A0A0E;font-family:'Segoe UI',sans-serif;">
  <div style="max-width:520px;margin:0 auto;background:#0D0F16;border-radius:16px;overflow:hidden;border:1px solid rgba(255,255,255,0.08);">
    <div style="background:linear-gradient(135deg,#4F8EF7 0%,#6B5EF0 60%,#A78BFA 100%);padding:28px 32px;position:relative;">
      <div style="position:absolute;inset:0;background:url('data:image/svg+xml,%3Csvg viewBox=\\'0 0 200 200\\' xmlns=\\'http://www.w3.org/2000/svg\\'%3E%3Cfilter id=\\'n\\'%3E%3CfeTurbulence type=\\'fractalNoise\\' baseFrequency=\\'0.85\\' numOctaves=\\'4\\'/%3E%3C/filter%3E%3Crect width=\\'100%25\\' height=\\'100%25\\' filter=\\'url(%23n)\\'/%3E%3C/svg%3E');opacity:0.04;mix-blend-mode:overlay"></div>
      <div style="position:relative;">
        <span style="font-size:22px;font-weight:900;color:#fff;letter-spacing:-0.5px;display:flex;align-items:center;gap:8px;"><span style="font-size:18px;">✦</span> PULSE</span>
        <p style="color:rgba(255,255,255,0.70);font-size:12px;margin:6px 0 0;font-weight:500;letter-spacing:0.04em;">WORKSPACE INVITATION</p>
      </div>
    </div>
    <div style="padding:32px;">
      <h2 style="color:#F0F0F2;font-size:20px;margin:0 0 10px;font-weight:700;">You've been invited to join ${workspaceName}</h2>
      <p style="color:#6A6A7A;font-size:14px;margin:0 0 24px;line-height:1.6;">
        <strong style="color:#A0A0B0;">${inviterName}</strong> has invited you to collaborate on <strong style="color:#A0A0B0;">${workspaceName}</strong> on Pulse.
      </p>
      <table style="width:100%;border-collapse:collapse;background:#141720;border-radius:10px;overflow:hidden;border:1px solid rgba(255,255,255,0.08);margin-bottom:24px;">
        <tr>
          <td style="padding:12px 16px;color:#52527A;font-size:12px;font-weight:600;border-bottom:1px solid rgba(255,255,255,0.06);">Workspace</td>
          <td style="padding:12px 16px;color:#E0E0EE;font-size:13px;border-bottom:1px solid rgba(255,255,255,0.06);">${workspaceName}</td>
        </tr>
        <tr>
          <td style="padding:12px 16px;color:#52527A;font-size:12px;font-weight:600;">Your role</td>
          <td style="padding:12px 16px;">
            <span style="background:rgba(79,142,247,0.15);color:#7EB3FF;border:1px solid rgba(79,142,247,0.25);border-radius:5px;padding:2px 10px;font-size:12px;font-weight:600;">${roleLabel}</span>
          </td>
        </tr>
      </table>
      <a href="${inviteUrl}" style="display:block;background:linear-gradient(135deg,#4F8EF7,#6B5EF0);color:#fff;text-decoration:none;padding:14px 24px;border-radius:10px;font-weight:700;font-size:14px;text-align:center;margin-bottom:20px;">Accept Invitation →</a>
      <p style="color:#3A3A52;font-size:11px;margin:0;line-height:1.6;text-align:center;">
        This invitation expires in 7 days. If you didn't expect this, you can ignore it.<br/>
        Or copy this link: <span style="color:#4F8EF7;word-break:break-all;">${inviteUrl}</span>
      </p>
    </div>
  </div>
</body>
</html>`
  return { subject: `${inviterName} invited you to join ${workspaceName} on Pulse`, html }
}
